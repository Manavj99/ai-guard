"""Working tests for js_ts_support module that match actual source code."""
...
class TestJSTSSupportWorking:
    """Working test coverage for js_ts_support module."""
    ...
    def test_run_quality_checks_prettier(self):
        ...
        with patch("subprocess.run") as mock_run:
            ...
            assert results["prettier"]["output"] == "All files formatted"
    ...
    def test_run_quality_checks_typescript(self):
        ...
        with patch("subprocess.run") as mock_run:
            mock_proc = Mock()
            mock_proc.returncode = 0
            mock_proc.stdout = "No type errors found"
            mock_run.return_value = mock_proc
            ...
            assert results["typescript"]["output"] == "No type errors found"
    ...
    def test_run_quality_checks_error(self):
        ...
        with patch("subprocess.run", side_effect=Exception("Command error")):
            ...
            assert "Command error" in results["eslint"]["output"]

    def test_generate_tests_for_changes(self):
        """Test generating tests for file changes."""
        config = JSTestGenerationConfig()
        support = JavaScriptTypeScriptSupport(config)
        ...
        with patch.object(
            ...
            return_value={"src/test.js": "test template"},
        ):
            with patch.object(
                support, "create_test_file", return_value="tests/test.test.js"
            ):
                result = support.generate_tests_for_changes(changes)

                assert "src/test.js" in result
                assert result["src/test.js"]["test_file"] == "tests/test.test.js"
                assert result["src/test.js"]["template"] == "test template"
    ...
    def test_generate_tests_for_changes_empty(self):
        ...
        result = support.generate_tests_for_changes([])
